...
export default function FloatingContactWidget() {
  ...
  return (
    <>
      ...
      {/* Floating Bottom Action Widget (matching reference image) */}
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center justify-center">
        <motion.div 
          ...
          className="relative flex items-center gap-3 sm:gap-4 bg-white/40 dark:bg-zinc-950/40 backdrop-blur-3xl border border-white/60 dark:border-white/25 px-4 sm:px-5 py-2.5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.18)] ring-1 ring-white/40 dark:ring-white/10 group cursor-pointer overflow-hidden"
        >
          ...
          {/* Left Text Label (hidden when hovered anywhere on widget) */}
          <AnimatePresence mode="wait">
            {!isWidgetHovered && (
              <motion.div 
                ...
                className="flex flex-col z-10 overflow-hidden shrink-0"
              >
          ...
          <div className="flex items-center gap-2 z-10">
            ...
            >
              ...
              {isWidgetHovered ? (
                ...
                <span className="hidden whitespace-nowrap font-extrabold text-xs tracking-tight relative z-10 sm:inline-block">
                  Book a Call
                </span>
              )}
            </motion.button>
